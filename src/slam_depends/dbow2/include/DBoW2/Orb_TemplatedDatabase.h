...
#include "Orb_TemplatedVocabulary.h"
...
public:
   ...
   */
  explicit Orb_TemplatedDatabase(bool use_di = true, int di_levels = 0);

  /**
   * Creates a database with the given vocabulary
   * @param T class inherited from Orb_TemplatedVocabulary<TDescriptor, F>
   ...
   */
  template<class T>
  explicit Orb_TemplatedDatabase(const T &voc, bool use_di = true, 
    int di_levels = 0);
   ...
   */
  Orb_TemplatedDatabase(const Orb_TemplatedDatabase<TDescriptor, F> &db);

  /** 
   * Creates the database from a file
   * @param filename
   */
  Orb_TemplatedDatabase(const std::string &filename);

  /** 
   * Creates the database from a file
   * @param filename
   */
  Orb_TemplatedDatabase(const char *filename);

  /**
   * Destructor
   */
  virtual ~Orb_TemplatedDatabase(void);

  /**
   * Copies the given database and its vocabulary
   * @param db database to copy
   */
  Orb_TemplatedDatabase<TDescriptor,F>& operator=(
    const Orb_TemplatedDatabase<TDescriptor,F> &db);
   ...
   * @return vocabulary
   */
  inline const Orb_TemplatedVocabulary<TDescriptor,F>* getVocabulary() const;
...
protected:
  ...
  Orb_TemplatedVocabulary<TDescriptor, F> *m_voc;
...
template<class TDescriptor, class F>
Orb_TemplatedDatabase<TDescriptor, F>::Orb_TemplatedDatabase
...
template<class TDescriptor, class F>
Orb_TemplatedDatabase<TDescriptor, F>::~Orb_TemplatedDatabase(void)
{
  delete m_voc;
}
...
template<class TDescriptor, class F>
Orb_TemplatedDatabase<TDescriptor,F>& Orb_TemplatedDatabase<TDescriptor,F>::operator=
  (const Orb_TemplatedDatabase<TDescriptor,F> &db)
{
  if(this != &db)
  {
...
template<class TDescriptor, class F>
EntryId Orb_TemplatedDatabase<TDescriptor, F>::add(
...
template<class TDescriptor, class F>
EntryId Orb_TemplatedDatabase<TDescriptor, F>::add(const BowVector &v,
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::delete_entry(const EntryId entry_id)
{
...
template<class TDescriptor, class F>
template<class T>
inline void Orb_TemplatedDatabase<TDescriptor, F>::setVocabulary
  (const T& voc)
{
  delete m_voc;
  m_voc = new T(voc);
...
template<class TDescriptor, class F>
inline const Orb_TemplatedVocabulary<TDescriptor,F>* 
Orb_TemplatedDatabase<TDescriptor, F>::getVocabulary() const
{
  return m_voc;
}
...
template<class TDescriptor, class F>
inline void Orb_TemplatedDatabase<TDescriptor, F>::clear()
{
  // resize vectors
  m_ifile.resize(0);
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::allocate(int nd, int ni)
{
...
template<class TDescriptor, class F>
inline unsigned int Orb_TemplatedDatabase<TDescriptor, F>::size() const
{
  return m_nentries;
}
...
template<class TDescriptor, class F>
inline bool Orb_TemplatedDatabase<TDescriptor, F>::usingDirectIndex() const
{
  return m_use_di;
}
...
template<class TDescriptor, class F>
inline int Orb_TemplatedDatabase<TDescriptor, F>::getDirectIndexLevels() const
{
  return m_dilevels;
}
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::query(
  const std::vector<TDescriptor> &features,
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::queryL1(const BowVector &vec, 
  QueryResults &ret, int max_results, int max_id) const
{
  BowVector::const_iterator vit;
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::queryL2(const BowVector &vec, 
  QueryResults &ret, int max_results, int max_id) const
{
  BowVector::const_iterator vit;
  typename IFRow::const_iterator rit;
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::queryChiSquare(const BowVector &vec, 
  QueryResults &ret, int max_results, int max_id) const
{
  BowVector::const_iterator vit;
  typename IFRow::const_iterator rit;
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::queryKL(const BowVector &vec, 
  QueryResults &ret, int max_results, int max_id) const
{
  BowVector::const_iterator vit;
  typename IFRow::const_iterator rit;
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::queryBhattacharyya(
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::queryDotProduct(
  const BowVector &vec, QueryResults &ret, int max_results, int max_id) const
{
  BowVector::const_iterator vit;
  typename IFRow::const_iterator rit;
...
template<class TDescriptor, class F>
const FeatureVector& Orb_TemplatedDatabase<TDescriptor, F>::retrieveFeatures
  (EntryId id) const
{
  assert(id < size());
  return m_dfile[id];
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::save(const std::string &filename) const
{
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::save(cv::FileStorage &fs,
  const std::string &name) const
{
  // Format YAML:
  // vocabulary { ... see Orb_TemplatedVocabulary::save }
  // database 
  // {
  //   nEntries: 
  //   usingDI: 
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::load(const std::string &filename)
{
...
template<class TDescriptor, class F>
void Orb_TemplatedDatabase<TDescriptor, F>::load(const cv::FileStorage &fs,
  const std::string &name)
{ 
  // load voc first
  // subclasses must instantiate m_voc before calling this ::load
  if(!m_voc) m_voc = new Orb_TemplatedVocabulary<TDescriptor, F>;
