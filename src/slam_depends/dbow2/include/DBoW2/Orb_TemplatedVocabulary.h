/**
 * File: Orb_TemplatedVocabulary.h
 * Date: February 2011
...
public:
   ...
   */
    Orb_TemplatedVocabulary(int k = 10, int L = 5,
   ...
   * @param filename
   */
    Orb_TemplatedVocabulary(const std::string &filename);

    /**
   * Creates the vocabulary by loading a file
   * @param filename
   */
    Orb_TemplatedVocabulary(const char *filename);

    /**
   * Copy constructor
   * @param voc
   */
    Orb_TemplatedVocabulary(const Orb_TemplatedVocabulary<TDescriptor, F> &voc);

    /**
   * Destructor
   */
    virtual ~Orb_TemplatedVocabulary();
   ...
   */
    Orb_TemplatedVocabulary<TDescriptor, F> &operator=(
        const Orb_TemplatedVocabulary<TDescriptor, F> &voc);
...
template <class TDescriptor, class F>
Orb_TemplatedVocabulary<TDescriptor, F>::Orb_TemplatedVocabulary(int k, int L, WeightingType weighting, ScoringType scoring)
...
template <class TDescriptor, class F>
Orb_TemplatedVocabulary<TDescriptor, F>::Orb_TemplatedVocabulary(const std::string &filename) : m_scoring_object(NULL)
{
#if VINS_BRIEF
    //m_scoring = KL;
    // Changed by VINS [[[
...
template <class TDescriptor, class F>
Orb_TemplatedVocabulary<TDescriptor, F>::Orb_TemplatedVocabulary(const char *filename) : m_scoring_object(NULL)
{
#if VINS_BRIEF
    //m_scoring = KL;
    // Changed by VINS [[[
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::createScoringObject()
{
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::setScoringType(ScoringType type)
{
    m_scoring = type;
    createScoringObject();
}
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::setWeightingType(WeightingType type)
{
...
template <class TDescriptor, class F>
Orb_TemplatedVocabulary<TDescriptor, F>::Orb_TemplatedVocabulary(
    const Orb_TemplatedVocabulary<TDescriptor, F> &voc)
    : m_scoring_object(NULL)
{
...
template <class TDescriptor, class F>
Orb_TemplatedVocabulary<TDescriptor, F> &
Orb_TemplatedVocabulary<TDescriptor, F>::operator=(const Orb_TemplatedVocabulary<TDescriptor, F> &voc)
{
    this->m_k = voc.m_k;
    this->m_L = voc.m_L;
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::create(
    const std::vector<std::vector<TDescriptor>> &training_features)
{
    m_nodes.clear();
    m_words.clear();
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::getFeatures(
    const std::vector<std::vector<TDescriptor>> &training_features,
    std::vector<pDescriptor> &features) const
{
    features.resize(0);
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::HKmeansStep(NodeId parent_id,
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::initiateClusters(const std::vector<pDescriptor> &descriptors,
                                                               std::vector<TDescriptor> &clusters) const
{
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::initiateClustersKMpp(
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::createWords()
{
    m_words.resize(0);
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::setNodeWeights(const std::vector<std::vector<TDescriptor>> &training_features)
{
...
template <class TDescriptor, class F>
inline unsigned int Orb_TemplatedVocabulary<TDescriptor, F>::size() const
{
    return m_words.size();
}
...
template <class TDescriptor, class F>
inline bool Orb_TemplatedVocabulary<TDescriptor, F>::empty() const
{
...
template <class TDescriptor, class F>
float Orb_TemplatedVocabulary<TDescriptor, F>::getEffectiveLevels() const
{
    long sum = 0;
...
template <class TDescriptor, class F>
TDescriptor Orb_TemplatedVocabulary<TDescriptor, F>::getWord(WordId wid) const
{
...
template <class TDescriptor, class F>
WordValue Orb_TemplatedVocabulary<TDescriptor, F>::getWordWeight(WordId wid) const
{
...
template <class TDescriptor, class F>
WordId Orb_TemplatedVocabulary<TDescriptor, F>::transform(const TDescriptor &feature) const
{
    if (empty())
    {
        return 0;
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::transform(
...
template <class TDescriptor, class F>
inline double Orb_TemplatedVocabulary<TDescriptor, F>::score(const BowVector &v1, const BowVector &v2) const
{
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::transform(const TDescriptor &feature, WordId &id) const
{
    WordValue weight;
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::transform(const TDescriptor &feature,
...
template <class TDescriptor, class F>
NodeId Orb_TemplatedVocabulary<TDescriptor, F>::getParentNode(WordId wid, int levelsup) const
{
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::getWordsFromNode(NodeId nid, std::vector<WordId> &words) const
{
    words.clear();
...
template <class TDescriptor, class F>
int Orb_TemplatedVocabulary<TDescriptor, F>::stopWords(double minWeight)
{
    int c = 0;
...
template <class TDescriptor, class F>
bool Orb_TemplatedVocabulary<TDescriptor, F>::loadFromTextFile(const std::string &filename)
{
    ifstream f;
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::saveToTextFile(const std::string &filename) const
{
    fstream f;
...
template <class TDescriptor, class F>
bool Orb_TemplatedVocabulary<TDescriptor, F>::loadFromBinaryFile(const std::string &filename)
{
    fstream f;
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::saveToBinaryFile(const std::string &filename) const
{
    fstream f;
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::save(const std::string &filename) const
{
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::load(const std::string &filename)
{
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::save(cv::FileStorage &f,
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::load(const cv::FileStorage &fs,
...
template <class TDescriptor, class F>
void Orb_TemplatedVocabulary<TDescriptor, F>::loadBin(const std::string &filename)
{
    m_words.clear();
    m_nodes.clear();
